...
/**
 * A <code>ClassLoader</code> which embeds a <code>Mutater</code> so that
 * applications can be run with a single class undergoing mutation.
 * 
 * @author Tin Pavlinic
...
public class MutatingClassLoader extends ClassLoader {
	...
	private final String[] mIgnoredPackages = new String[] { "java.",
	// "javax.",
			"sun.reflect", "junit.",
	// "org.apache",
	// "org.xml",
	 ...
	 * 
	 * @param target
	 *            the class name to be mutated. Other classes will not be
	 *            mutated.
	 * @param mutater
	 *            a <code>Mutater</code> value that will carry out mutations.
	 * @param classpath
	 *            a <code>String</code> value supplying the classes visible to
	 *            the classloader.
	 */
	public MutatingClassLoader(final String target, final Mutater mutater,
			final String classpath) {
		// Add these ignored classes to work around jakarta commons logging
		// stupidity with class loaders.
		mTarget = target;
		mMutater = mutater;
	...
	public int countMutationPoints(String className)
		...
		if ((cl = (Class) mClasses.get(className)) == null) {
			...
			for (int i = 0; i < mIgnoredPackages.length; i++) {
				if (className.startsWith(mIgnoredPackages[i])) {
					// System.err.println("Parent forced loading of class: " +
					// className);
			...
			if (cl == null) {
				...
				if (clazz != null) {
					...
					// System.err.println("Parent loading of class: " +
					// className);
					cl = mDeferTo.loadClass(className);
				}
			}
	...
	/**
	 * If the class matches the target then it is mutated, otherwise the class
	 * if returned unmodified. Overrides the corresponding method in the
	 * superclass. Classes are cached so that we always load a fresh version.
	 * 
	...
	public InputStream getResourceAsStream(String name) {
		...
		try {
			resource = new ClassPathResource(name).getInputStream();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
